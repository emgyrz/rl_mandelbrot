...
fn wrte_image(filename: &str, pixels: &[u8], bounds: (usize, usize)) -> Result<(), io::Error> {
...
fn main() {
    ...
    if args.len() != 5 {
        writeln!(io::stderr(), "Неверный вызов комманды");
        writeln!(
            io::stderr(),
    ...
    wrte_image(&args[1], &pixels, bounds).expect("ошибка при записи PNG")
}
